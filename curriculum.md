# Kotlin + Spring Boot 백엔드 전환 커리큘럼

대상: Java + Spring Boot 4년차 백엔드 개발자  
목표: Kotlin + Spring Boot 기반 이커머스 백엔드 실무 전환  
핵심 스택: Kotlin, Spring Boot, JPA, QueryDSL, MySQL/MariaDB, Redis, Kafka(optional), AWS 배포  
IDE: IntelliJ IDEA  
최종 산출물: 이커머스 주문/정산/재고 도메인 기반 실무형 프로젝트 1개  
...
- 3단계: JPA + QueryDSL 기반 조회/저장 구조 구현
- 4단계: 이커머스 핵심 도메인 구현
- 5단계: 트랜잭션/동시성/테스트
- 6단계: 운영 관점 고도화
...
- Java/Kotlin 버전 설정
- Spring Web / Spring Data JPA / Validation / QueryDSL / MySQL Driver 의존성 추가
- application.yml 작성
- 로컬 DB 연결
- 패키지 구조 생성
...
```md
너는 Kotlin + Spring Boot 학습을 돕는 시니어 백엔드 리뷰어다.
...
```md
너는 시니어 Kotlin + Spring Boot 백엔드 엔지니어이자 코드 리뷰어다.

목표는 Kotlin + Spring Boot 기반 이커머스 백엔드 프로젝트를 step by step으로 구현하면서 학습하는 것이다.
...
6. JPA + QueryDSL 확장성을 반드시 고려한다.
7. 이커머스 도메인(Product, Order, Settlement)에 자연스럽게 이어질 수 있게 설계한다.
...
5. STEP 00-05. docker mysql 실행 환경 구성
